...
const CONFIG = {
  ...
  WEBHOOK_URL: process.env.WEBHOOK_URL || 'http://localhost:8080/v1/pagos/webhook'  // URL por defecto para evitar configuración manual
};
...
app.post('/procesar-pago', async (req, res) => {
  ...
  const cardInfo = TEST_CARDS[cardClean] || { status: 'rejected', name: 'Tarjeta Desconocida/Inválida' };
...
function getPaymentPage(txn) {
  return '<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Pagar - PlusPagos</title>' +
    ...
    'input{width:100%;padding:14px;border:2px solid #e2e8f0;border-radius:8px;font-size:1rem;letter-spacing:0.05em}input:focus{outline:none;border-color:#3b82f6}' +
    ...
    '.hint{color:#94a3b8;font-size:0.8rem;margin-top:20px;text-align:center}' +
    '</style></head><body>' +
    '<div class="card"><h1>PlusPagos</h1><p style="color:#64748b">Completá los datos de tu tarjeta</p>' +
    '<div class="amount">$ ' + txn.monto + '</div>' +
    '<form action="/procesar-pago" method="POST" id="payForm">' +
    '<input type="hidden" name="transactionId" value="' + txn.id + '">' +
    '<input type="hidden" id="cardNumberHidden" name="cardNumber">' +
    '<div class="form-group"><label>Número de tarjeta</label><input type="text" id="cardNumberDisplay" placeholder="4242 4242 4242 4242" required maxlength="19" autocomplete="cc-number"></div>' +
    ...
    '<div class="row">' +
    '<div class="form-group"><label>Vencimiento</label><input type="text" name="cardExpiry" id="cardExpiry" placeholder="MM/AA" required maxlength="5" autocomplete="cc-exp"></div>' +
    '<div class="form-group"><label>CVV</label><input type="text" name="cardCvv" id="cardCvv" placeholder="123" required maxlength="4" autocomplete="cc-csc"></div></div>' +
    '<button type="submit" class="btn" id="btnPagar">Pagar $ ' + txn.monto + '</button></form>' +
    ...
    '  var raw=e.target.value.replace(/\\D/g,"").substring(0,16);' +
    '  var fmt=raw.replace(/(\\d{4})(?=\\d)/g,"$1 ");' +
    '  e.target.value=fmt;' +
    '  hidden.value=raw;' +
    '});' +
    // Expiry: auto-slash after 2 digits
    ...
    '  else if(raw.length===2 && e.inputType!=="deleteContentBackward"){raw=raw+"/";}' +
    '  e.target.value=raw;' +
    '});' +
    // CVV: digits only
